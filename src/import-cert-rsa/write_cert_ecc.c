...
int ImportClientKey(void *hSessionHandle) {
  ...
  int len1 = read_content_from_file("/vagrant/cert/ecc/all/client.crt", &ssl_client_cert);
  ...
  ret = SDF_ImportSignPublicKey_ECC(hSessionHandle, uiKeyIndex, &publicKey);
  ...
  ret = SDF_ImportSignPrivateKey_ECC(hSessionHandle, uiKeyIndex, &privateKey);
...
int WriteClientCert(void *hSessionHandle) {
  ...
  read_content_from_file("/vagrant/cert/ecc/all/client.crt", &ssl_client_ca);

  ret = SDF_CreateFile(hSessionHandle, "ecc-client.crt", strlen("ecc-client.crt"), strlen(ssl_client_ca));
  ...
  ret = SDF_WriteFile(hSessionHandle, "ecc-client.crt", strlen("ecc-client.crt"), 1, strlen(ssl_client_ca),
                      ssl_client_ca);
  ...
  char ssl_client_key1[2976] = {0};
  ret = SDF_ReadFile(hSessionHandle, "ecc-client.crt", strlen("ecc-client.crt"), 1, &readLen,
                     ssl_client_key1);
...
int WriteCaCert(void *hSessionHandle) {
  ...
  char *ssl_client_ca = NULL;
  read_content_from_file("/vagrant/cert/ecc/all/ca.crt", &ssl_client_ca);

  ret = SDF_CreateFile(hSessionHandle, "ecc-ca.crt", strlen("ecc-ca.crt"), strlen(ssl_client_ca));
  printf("SDF_CreateFile ret=0x%08x\n", ret);
  if (SDR_OK != ret) {
    printf("SDF_CreateFile\n");
    goto EXIT;
  ...
  ret = SDF_WriteFile(hSessionHandle, "ecc-ca.crt", strlen("ecc-ca.crt"), 1, strlen(ssl_client_ca),
                      ssl_client_ca);
  printf("SDF_WriteFile ret=0x%08x\n", ret);
  if (SDR_OK != ret) {
    printf("SDF_WriteFile\n");
  ...
  int readLen = strlen(ssl_client_ca);  //3868
  char ssl_client_key1[5481] = {0};
  ret = SDF_ReadFile(hSessionHandle, "ecc-ca.crt", strlen("ecc-ca.crt"), 1, &readLen,
                     ssl_client_key1);
