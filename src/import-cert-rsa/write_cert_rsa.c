...
int ImportGcyClientKey(void *hSessionHandle) {
    int ret;
    int bits;
    int i, j;
    unsigned int uiKeyIndex = 8;
    RSArefPublicKey publicKey = {0};
    RSArefPrivateKey privateKey = {0};
    unsigned char datain[2 * 1024] = {0};
    unsigned char dataout[2 * 1024] = {0};
    ...
    char *ssl_client_key = NULL;
    unsigned int len = read_content_from_file("/vagrant/cert/rsa_gcy/all/client.key",
                                              &ssl_client_key);
    ...
    char *ssl_client_cert = NULL;
    int len1 = read_content_from_file("/vagrant/cert/rsa_gcy/all/client.crt",
                                      &ssl_client_cert);
    BIO *bio_mem = BIO_new(BIO_s_mem());
    BIO_puts(bio_mem, ssl_client_cert);
    X509 *cert = PEM_read_bio_X509(bio_mem, NULL, NULL, NULL);
...
int WriteGcyClientCert(void *hSessionHandle) {
    int ret;

    char *ssl_client_ca = NULL;
    read_content_from_file("/vagrant/cert/rsa_gcy/all/client.crt", &ssl_client_ca);

    ret = SDF_CreateFile(hSessionHandle, "client_gcy.crt", strlen("client_gcy.crt"),
                         strlen(ssl_client_ca));
    printf("SDF_CreateFile ret=0x%08x\n", ret);
    if (SDR_OK != ret) {
        printf("SDF_CreateFile\n");
    ...
    ret = SDF_WriteFile(hSessionHandle, "client_gcy.crt", strlen("client_gcy.crt"), 0,
                        strlen(ssl_client_ca), ssl_client_ca);
    printf("SDF_WriteFile ret=0x%08x\n", ret);
    if (SDR_OK != ret) {
        printf("SDF_WriteFile\n");
    ...
    int readLen = strlen(ssl_client_ca); // 3868
    char ssl_client_key1[5018] = {0};
    ret = SDF_ReadFile(hSessionHandle, "client_gcy.crt", strlen("client_gcy.crt"), 0,
                       &readLen, ssl_client_key1);
...
int WriteGcyCaCert(void *hSessionHandle) {
    int ret;

    char *ssl_client_ca = NULL;
    read_content_from_file("/vagrant/cert/rsa_gcy/all/ca.crt", &ssl_client_ca);

    ret = SDF_CreateFile(hSessionHandle, "ca_gcy.crt", strlen("ca_gcy.crt"),
                         strlen(ssl_client_ca));
    printf("SDF_CreateFile ret=0x%08x\n", ret);
    if (SDR_OK != ret) {
        printf("SDF_CreateFile\n");
    ...
    ret = SDF_WriteFile(hSessionHandle, "ca_gcy.crt", strlen("ca_gcy.crt"), 0,
                        strlen(ssl_client_ca), ssl_client_ca);
    printf("SDF_WriteFile ret=0x%08x\n", ret);
    if (SDR_OK != ret) {
        printf("SDF_WriteFile\n");
    ...
    int readLen = strlen(ssl_client_ca); // 3868
    char ssl_client_key1[13538] = {0};
    ret = SDF_ReadFile(hSessionHandle, "ca_gcy.crt", strlen("ca_gcy.crt"), 0, &readLen,
                       ssl_client_key1);
...
int WriteCert() {
    ...
    ret = WriteGcyCaCert(hSessionHandle);

    if (SDR_OK != ret) {
        printf("WriteGcyCaCert error=%d\n", ret);
        return -1;
    }
    ...
    ret = ImportGcyClientKey(hSessionHandle);
    if (SDR_OK != ret) {
        printf("ImportGcyClientKey error=%d\n", ret);
        return -1;
    }
    ...
    ret = WriteGcyClientCert(hSessionHandle);

    if (SDR_OK != ret) {
        printf("WriteGcyClientCert error=%d\n", ret);
        return -1;
    }
